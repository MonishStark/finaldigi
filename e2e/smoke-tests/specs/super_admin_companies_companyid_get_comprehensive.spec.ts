...
/**
 * Comprehensive test suite for GET /super-admin/companies/{companyId} endpoint
 * Fetch company details (super-admin only)
 * Spec Response Codes: 200, 401 (3 variants), 403, 404
 */
...
test.describe("GET /super-admin/companies/{companyId} - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should fetch company details successfully", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/companies/test-company-id`,
				{
					headers: {
		...
		test("should return company with required fields", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/companies/test-company-id`,
				{
					headers: {
	...
	test.describe("401 Authentication Error Responses", () => {
		test("should return 401 when missing access token", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/companies/test-id`,
				{
					headers: {
						"Content-Type": "application/json",
					},
	...
	test.describe("404 Not Found Responses", () => {
		test("should return 404 when company not found", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/companies/nonexistent-id`,
				{
					headers: {
						Authorization: `Bearer ${validAccessToken}`,
						"Content-Type": "application/json",
