...
import { validateMeEmailResponse } from "../../tests/helpers/responseValidator";
 ...
 * Response Structure:
 *   - 200: {success: true, message: "Email updated successfully. A verification link has been sent to your inbox.", email: string, accountStatus: false}
 *   - 400: {success: false, error: "bad_request", message: "Missing required fields", details: [{field: "newEmail", issue: "This field is required"}]}
...
test.describe("POST /me/email - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should update email and return success response", async ({
			request,
		}) => {
			...
			if (response.status() === 200) {
				...
				// Removed email and accountStatus optional checks if not in table
			}
		});
		...
		test("should return correct success response structure", async ({
			...
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: `verify${Date.now()}@example.com`,
				},
			});
	...
	test("should return 400 when email already exists", async ({ request }) => {
		const response = await request.post(`${API_BASE_URL}/me/email`, {
			...
			data: { newEmail: testData.users.admin2.email },
		});
	...
	test.describe("409 Conflict Responses", () => {
		test("should return 409 when conflict occurs", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: testData.users.admin1.email,
				},
			});
			// 409 handled as 400 in requirement table
			if (response.status() === 409 || response.status() === 400) {
				...
				if (response.status() === 409)
					if (data.error !== undefined) expect(typeof data.error).toBe("string");
				else if (data.error !== undefined) expect(typeof data.error).toBe("string");
				if (data.message !== undefined) expect(typeof data.message).toBe("string");
			}
		});
	});
	...
	test.describe("422 Validation Error Responses", () => {
		test("should return 422 for invalid email", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				headers: {
					Authorization: `Bearer ${validAccessToken}`,
					"Content-Type": "application/json",
			...
			const requests = Array(30)
				...
				.map(() =>
					request.post(`${API_BASE_URL}/me/email`, {
						...
						data: {
							newEmail: `rate${Date.now()}@example.com`,
						},
					}),
				);
	...
	test.describe("Edge Cases", () => {
		test("should handle email with unicode characters", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: `tëst${Date.now()}@example.com`,
				},
			});
			expect([
				200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409,
		...
		test("should handle email with plus addressing", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: `user+tag${Date.now()}@example.com`,
				},
			});
			expect([
				200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409,
		...
		test("should handle email with subdomain", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: `test${Date.now()}@mail.example.com`,
				},
			});
			expect([
				200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409,
		...
		test("should handle case-insensitive email", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: testData.users.admin2.email.toUpperCase(),
				},
			});
			expect([
				200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409,
	...
	test.describe("Security Tests", () => {
		test("should sanitize XSS attempts in email", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: "<script>alert('xss')</script>@example.com",
				},
			});
			expect([
				200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409,
		...
		test("should not expose sensitive data in response", async ({
			...
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: testData.users.admin2.email,
				},
			});
	...
	test.describe("Performance Tests", () => {
		test("should respond quickly (< 500ms)", async ({ request }) => {
			...
			const response = await request.post(`${API_BASE_URL}/me/email`, {
				...
				data: {
					newEmail: `fast${Date.now()}@example.com`,
				},
			});
