...
import { validateMePasswordResponse } from "../../tests/helpers/responseValidator";
...
/**
 * Comprehensive test suite for POST /me/password endpoint
 ...
 *   - 400: {success: false, error: "bad_request", message: "Missing required fields", details: [{field: "currentPassword", issue: "This field is required"}]}
...
test.describe("POST /me/password - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should return success when password is updated", async ({
			...
			const response = await request.post(`${API_BASE_URL}/me/password`, {
				headers: {
					Authorization: `Bearer ${validAccessToken}`,
					"Content-Type": "application/json",
				},
				data: {
					currentPassword: testData.users.admin1.password,
					newPassword: `NewTest@${Date.now()}1`,
				},
			});
		...
		test("should return correct success response structure", async ({
			...
			const response = await request.post(`${API_BASE_URL}/me/password`, {
				...
				data: {
					currentPassword: testData.users.admin1.password,
					newPassword: `NewTest@${Date.now()}2`,
				},
			});
		...
		test("should return 400 when currentPassword is missing", async ({
			...
			if (Array.isArray(data.details) && data.details.length > 0) {
				...
				expect(data.details[0].issue).toBe(
					"currentPassword/newPassword is required",
				);
			}
		});
	});
	...
	test.describe("405 Method Not Allowed Responses", () => {
		test("should return 405 for GET method", async ({ request }) => {
			const response = await request.get(`${API_BASE_URL}/me/password`, {
				headers: {
					Authorization: `Bearer ${validAccessToken}`,
				},
			});
	...
	test.describe("422 Validation Error Responses", () => {
		test("should return 422 for invalid new password", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/me/password`, {
				headers: {
					Authorization: `Bearer ${validAccessToken}`,
					"Content-Type": "application/json",
	...
	test.describe("423 Locked Responses", () => {
		test("should return 423 when account is locked", async ({ request }) => {
			...
			if (![200, 401, 423].includes(response.status())) return;
	...
	test.describe("429 Rate Limit Responses", () => {
		test("should return 429 when rate limit is exceeded", async ({
			...
			const requests = Array(30)
				.fill(null)
				.map(() =>
					request.post(`${API_BASE_URL}/me/password`, {
						headers: {
							Authorization: `Bearer ${validAccessToken}`,
							"Content-Type": "application/json",
						},
						data: {
							currentPassword: testData.users.admin1.password,
							newPassword: `NewTest@${Date.now()}3`,
						},
					}),
				);
