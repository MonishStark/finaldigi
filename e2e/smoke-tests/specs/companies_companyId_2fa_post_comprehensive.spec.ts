...
import { validateCompaniesTwoFactorResponse } from "../../tests/helpers/responseValidator";
 ...
 * Response Structure:
 *   - 200: Scenario A — Everything succeeds
...
test.describe("POST /companies/{companyId}/2fa - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should enable 2FA for company user", async ({ request }) => {
			...
			expect(data.message).toContain("Two-factor authentication");
	...
	test.describe("400 Bad Request Responses", () => {
		test("should return 400 when required fields are missing", async ({
			...
			if (Array.isArray(data.details) && data.details.length > 0) {
				expect(data.details[0].field).toBe("enabled");
	...
	test.describe("404 Not Found Responses", () => {
		test("should return 404 when user is not found", async ({ request }) => {
			const response = await request.post(
				`${API_BASE_URL}/companies/999999/2fa`,
				{
					headers: {
						Authorization: `Bearer ${validAccessToken}`,
						"Content-Type": "application/json",
	...
	test.describe("Edge Cases", () => {
		test("should handle concurrent 2FA updates", async ({ request }) => {
			...
			responses.forEach((response) => {
				responses.forEach((response) => {
					expect([
						200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408,
