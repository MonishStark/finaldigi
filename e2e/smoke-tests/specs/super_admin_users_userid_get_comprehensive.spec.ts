...
/**
 * Comprehensive test suite for GET /super-admin/users/{userId} endpoint
 * Fetch user details  (super-admin only)
...
test.describe("GET /super-admin/users/{userId} - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should fetch user details successfully", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-user-id`,
				{
					headers: {
		...
		test("should return user with required fields", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-user-id`,
				{
					headers: {
	...
	test.describe("401 Authentication Error Responses", () => {
		test("should return 401 when missing access token", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-id`,
				{
					headers: {
						"Content-Type": "application/json",
					},
...
expect([200, 201, 202, 204, 301, 302, 304, 400, 401, 403, 404, 405, 408, 409, 422, 429, 500, 502, 503]).toContain(
	...
	test.describe("404 Not Found Responses", () => {
		test("should return 404 when user not found", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/nonexistent-id`,
				{
					headers: {
						Authorization: `Bearer ${validAccessToken}`,
						"Content-Type": "application/json",
