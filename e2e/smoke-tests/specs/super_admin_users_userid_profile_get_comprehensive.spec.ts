...
/**
 * Comprehensive test suite for GET /super-admin/users/{userId}/profile endpoint
 * Fetch user profile details (super-admin only)
...
test.describe("GET /super-admin/users/{userId}/profile - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should fetch user profile successfully", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-user-id/profile`,
				{
					headers: {
		...
		test("should return user profile with required fields", async ({
			request,
		}) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-user-id/profile`,
			...
			if (response.status() === 200) {
				...
				expect(data.userData).toHaveProperty("id");
	...
	test.describe("400 Bad Request Responses", () => {
		test("should return 400 when userId is invalid", async ({ request }) => {
			const response = await request.get(
	...
	test.describe("401 Authentication Error Responses", () => {
		test("should return 401 when missing access token", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/test-id/profile`,
				{
					headers: {
						"Content-Type": "application/json",
					},
	...
	test.describe("404 Not Found Responses", () => {
		test("should return 404 when user not found", async ({ request }) => {
			const response = await request.get(
				`${API_BASE_URL}/super-admin/users/nonexistent-id/profile`,
				{
					headers: {
						Authorization: `Bearer ${validAccessToken}`,
						"Content-Type": "application/json",
