...
test.describe("PATCH /notifications/viewed - Comprehensive Tests", () => {
	...
	test.describe("200 Success Responses", () => {
		test("should mark all notifications as viewed", async ({ request }) => {
			const response = await request.patch(`${API_BASE_URL}/notifications/viewed`, {
				headers: {
	...
	test.describe("405 Method Not Allowed Responses", () => {
		test("should return 405 for POST method", async ({ request }) => {
			const response = await request.post(`${API_BASE_URL}/notifications/viewed`, {
				headers: {
					Authorization: `Bearer ${validAccessToken}`,
				},
			});
