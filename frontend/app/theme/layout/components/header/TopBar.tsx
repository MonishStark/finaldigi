...
export const TopBar: React.FC = () => {
  const { auth, istextEditor, teamList } = useAuth();
  ...
  return (
    <div id='kt_app_header' className='app-header'>
      ...
      >
            ...
            <div className='d-lg-flex px-4 align-items-center '>
              <div className=' align-items-center  d-none d-lg-flex'>
                {auth?.user?.role !== 4 ? (
                  ...
                  <div
                    className='py-4'
                    title={intl.formatMessage({ id: 'DOCUMENTS.RECORD_AUDIO' })}
                  >
                    {auth?.user?.role !== 3 &&
                      <AudioSummarizer />
                    }
                  ...
                  <SidebarMenuItem
                    ...
                    title={intl.formatMessage({ id: 'NOTIFICATIONS.NOTIFICATIONS' })}
                  ...
                  <>
                    <SidebarMenuItem
                      to='/admin/dashboard'
                      icon='element-11'
                      title={intl.formatMessage({ id: 'SUPERADMIN.DASHBOARD' })}
                      fontIcon='bi-app-indicator'
                      istextEditor={istextEditor}
                    />
                    <SidebarMenuItem
                      to='/admin/settings'
                      icon='code'
                      title={intl.formatMessage({ id: 'SUPERADMIN.CONFIGURATION' })}
                      fontIcon='bi-app-indicator'
                      istextEditor={istextEditor}
                    />
                    <SidebarMenuItem
                      to='/admin/clients'
                      icon='user'
                      title={intl.formatMessage({ id: 'SUPERADMIN.CLIENTS' })}
                      fontIcon='bi-app-indicator'
                      istextEditor={istextEditor}
                    />
                    <SidebarMenuItem
                      ...
                      icon='message-text-2'
                      title={intl.formatMessage({ id: 'SUPERADMIN.EMAIL_TEMPLATES' })}
                      fontIcon='bi-app-indicator'
                      istextEditor={istextEditor}
                    />
                  </>
        ...
        <Link to={`${auth?.user?.role === 4 ? '/admin/dashboard': teamList.length === 0 ? "/dashboard" : '/teams'}`}>
